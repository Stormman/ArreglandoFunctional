...
public struct List<A> {
	...
	public init(_ head : A, _ tail : List<A> = List<A>()) {
		if tail.count == -1 {
            ...
            let nuevoCount = tail.count + 1
            
			self.count = nuevoCount //tail.count.successor()
		}
		self.next = { (head, tail) }
	}
...
public func isSuffixOf<A : Equatable>(l : List<A>) -> (List<A>) -> Bool {
    return { r in isPrefixOf(l: l.reverse())(r.reverse()) }
}
...
public func isInfixOf<A : Equatable>(l : List<A>) -> (List<A>) -> Bool {
    return { r in any(isPrefixOf(l: l))(tail(r)) }
}
*/
...
public func stripPrefix<A : Equatable>(l : List<A>) -> (List<A>) -> Maybe<List<A>> {
	return { r in
		switch (l.match(), r.match()) {
		case (.Nil, _):
            return Maybe.just(x: r)
		case (.Cons(let x, let xs), .Cons(let y, let ys)) where x == y:
            return stripPrefix(l: xs)(ys)
		default:
...
extension List : Functor__ {
    public typealias B = Any
	typealias FB = List<B>

	public static func fmap<B>(f: @escaping(A) -> B) -> (List<A>) -> List<B> {
		return { $0.map(f) }
	}
}

public func <% <A, B>(x : A, l : List<B>) -> List<A> {
    return List.fmap(f: const(x))(l)
}

extension List : Pointed__ {
	public static func pure(x : A) -> List<A> {
		return List(x)
	}
}

extension List : Applicative__ {
...
public func <%> <A, B>(f :@escaping( A) -> B, ar : List<A>) -> List<B> {
    return List.fmap(f: f)(ar)
}

public func <*> <A, B>(a : List<(A) -> B> , l : List<A>) -> List<B> {
    return List.ap(a: a)(l)
}
...
protocol Alternative__ {}

extension List : Alternative__ {
...
/*
	public func some(v : List<A>) -> List<[A]> {
        return curry(<|) <%> v <*> many(v: v)
	}
*//*
	public func many(v : List<A>) -> List<[A]> {
        return some(v: v) <|> List<[A]>.pure(x: [])
	}*/
	/*
	public func optional(v : List<A>) -> List<Maybe<A>> {
        return Maybe.just <%> v <|> List<Maybe<A>>.pure(x: Maybe.nothing())
	}*/
}
...
extension List : Monad__ {
	
    public func bind<B>(f : @escaping (A) -> List<B>) -> List<B> {
...
public func >> <A, B>(x : List<A>, y : List<B>) -> List<B> {
    return x.bind(f: { (_) in
		return y
	})
}

protocol MonadPlus__{}

extension List : MonadPlus__ {
...
*/
	public func mzipWith<B, C>(f :  @escaping (A) -> (B) -> C) -> (List<A>) -> (List<B>) -> List<C> {
		return zipWith(f)
	}
