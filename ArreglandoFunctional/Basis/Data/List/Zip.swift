...
public func zip<A, B>(_ l : [A]) -> ([B]) -> [(A, B)] {
	return { l2 in
...
public func zip3<A, B, C>(_ l : [A]) -> ([B]) -> ([C]) -> [(A, B, C)] {
	return { l2 in { l3 in
...
public func zipWith3<A, B, C, D>(_ f :  @escaping (A) ->( B) -> (C) -> D) -> ([A]) -> ([B]) -> ([C]) -> [D] {
...
public func unzip<A, B>(_ l : [(A, B)]) -> ([A], [B]) {
	switch match(l) {
		case .Nil:
			return ([], [])
...
public func unzip3<A, B, C>(_ l : [(A, B, C)]) -> ([A], [B], [C]) {
	switch match(l) {
		case .Nil:
			return ([], [], [])
...
public func zip<A, B>(_ l : List<A>) -> (List<B>) -> List<(A, B)> {
	return { l2 in
...
public func zip3<A, B, C>(_ l : List<A>) -> (List<B>) -> (List<C>) -> List<(A, B, C)> {
	return { l2 in { l3 in
...
public func zipWith<A, B, C>(_ f :  @escaping(A) -> (B) -> C) -> (List<A>) -> (List<B>) -> List<C> {
	return { l in { l2 in
		switch (l.match(), l2.match()) {
			case (.Cons(let a, let as_), .Cons(let b, let bs)):
				return f(a)(b) <| zipWith(f)(as_)(bs)
...
public func zipWith3<A, B, C, D>(_ f : @escaping (A) -> (B) -> (C) -> D) -> (List<A>) -> (List<B>) -> (List<C>) -> List<D> {
	return { l in { l2 in { l3 in
		switch (l.match(), l2.match(), l3.match()) {
			case (.Cons(let a, let as_), .Cons(let b, let bs), .Cons(let c, let cs)):
				return f(a)(b)(c) <| zipWith3(f)(as_)(bs)(cs)
...
public func unzip<A, B>(_ l : List<(A, B)>) -> (List<A>, List<B>) {
	switch l.match() {
		case .Nil:
			return (List(), List())
		case .Cons(let (a, b), let tl):
...
public func unzip3<A, B, C>(_ l : List<(A, B, C)>) -> (List<A>, List<B>, List<C>) {
	switch l.match() {
		case .Nil:
