...
public struct ST<S, A> {	
	private let apply:(_ s: World<RealWorld>) -> (World<RealWorld>, A)
...
public func <%> <S, A, B>(_ f:  @escaping(A) -> B, st: ST<S, A>) -> ST<S, B> {
	return ST.fmap(f)(st)
}
...
public func <*> <S, A, B>(_ stfn: ST<S, (A) -> B>, st: ST<S, A>) -> ST<S, B> {
...
extension ST : Monad__  {
	public func bind<B>(_ f: (A) -> ST<S, B>) -> ST<S, B> {
		return f(runST())
	}
}

public func >>- <S, A, B>(_ x : ST<S, A>, f : (A) -> ST<S, B>) -> ST<S, B> {
	return x.bind(f)
}
...
public func -<< <S, A, B>(_ f : (A) -> ST<S, B>, xs : ST<S, A>) -> ST<S, B> {
	return xs.bind(f)
}

public func >-> <S, A, B, C>(_ f :  @escaping(A) -> ST<S, B>, g :  @escaping(B) -> ST<S, C>) -> (A) -> ST<S, C> {
	return { x in f(x) >>- g }
}
